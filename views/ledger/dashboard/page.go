...
func buildRecentJournalActivity(entries []*journalentrypb.JournalEntry, l fycha.LedgerDashboardLabels) []types.ActivityItem {
	if len(entries) == 0 {
		return nil
	}
...
func buildJournalsHTML(entries []*journalentrypb.JournalEntry, l fycha.LedgerDashboardLabels, routes fycha.JournalRoutes) template.HTML {
	if len(entries) == 0 {
		return ""
	}
