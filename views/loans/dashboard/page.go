...
func buildRecentPaymentsList(payments []*loanpaymentpb.LoanPayment, l fycha.LoanDashboardLabels) []types.ActivityItem {
	if len(payments) == 0 {
		return nil
	}
...
func buildTopLoansHTML(rows []LoanRow, l fycha.LoanDashboardLabels, routes fycha.LoanRoutes) template.HTML {
	if len(rows) == 0 {
		return ""
	}
