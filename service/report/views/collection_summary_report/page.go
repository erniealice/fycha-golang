...
//
// 20260520 Wave B P1.E.1 — `DB report.DataSource` replaced with the typed
...
type PageData struct {
	...
	Labels            report.CollectionSummaryReportLabels
...
// NewView creates the collection summary report pivot-table view.
func NewView(deps *Deps) view.View {
	return view.ViewFunc(func(ctx context.Context, viewCtx *view.ViewContext) view.ViewResult {
		...
		clientCategoryID := viewCtx.QueryParams["client-category-id"]
		locationID := viewCtx.QueryParams["location-id"]
		locationAreaID := viewCtx.QueryParams["location-area-id"]
		collectionMethodID := viewCtx.QueryParams["collection-method-id"]
		collectionType := viewCtx.QueryParams["collection-type"]

		reportURL := viewCtx.CurrentPath
		if reportURL == "" {
			reportURL = deps.Routes.CollectionSummaryReportURL
		}
		...
		if viewCtx.QueryParams["sheet"] == "filters" {
			...
			return view.OK("collection-summary-report-filter-sheet", &CollectionSummaryFilterSheetData{
				Filter:           sheetFilter,
				PeriodLabels:     pl,
				Labels:           l,
				ReportURL:        reportURL,
		...
		exportURL := buildExportURL(deps.Routes.CollectionSummaryReportExportURL, primary, rows, period, startDateStr, endDateStr)
		...
		pageData := &PageData{
			PageData: types.PageData{
				...
				ActiveNav:    "report",
				ActiveSubNav: "collection-summary-report",
				HeaderTitle:  l.PageTitle,
				HeaderIcon:   "icon-bar-chart",
				CommonLabels: deps.CommonLabels,
			},
			ContentTemplate:   "collection-summary-report-content",
			Labels:            l,
			Summary:           summary,
			Table:             table,
			Filter:            filter,
		...
		if viewCtx.Translations != nil {
			if provider, ok := viewCtx.Translations.(*lynguaV1.TranslationProvider); ok {
				if kb, _ := provider.LoadKBIfExists(viewCtx.Lang, viewCtx.BusinessType, "report-collection-summary-report"); kb != nil {
		...
		if viewCtx.IsHTMX {
			return view.OK("collection-summary-report-content", pageData)
		}
		return view.OK("collection-summary-report", pageData)
	})
}

// CollectionSummaryFilterSheetData holds data for the collection summary report filter sheet template.
type CollectionSummaryFilterSheetData struct {
	Filter           report.FilterState
	PeriodLabels     report.PeriodLabels
	Labels           report.CollectionSummaryReportLabels
	ReportURL        string
...
func buildSummary(s *aragingpb.CollectionSummarySummary, l report.CollectionSummaryReportLabels) []report.SummaryMetric {
	if s == nil {
...
func buildPivotTable(resp *aragingpb.GetCollectionSummaryResponse, l report.CollectionSummaryReportLabels, tableLabels types.TableLabels, primary, rowDim string) *types.TableConfig {
	...
	table := &types.TableConfig{
		ID:              "collectionSummaryReportTable",
