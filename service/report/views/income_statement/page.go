...
type IncomeStatementPageData struct {
	...
	EndDate       string
	PeriodLabel   string
	PeriodPresets []report.FilterOption
...
func NewIncomeStatementView(deps *IncomeStatementDeps) view.View {
	return view.ViewFunc(func(ctx context.Context, viewCtx *view.ViewContext) view.ViewResult {
		...
		start, end := report.ParsePeriodPreset(preset)
		if preset == "custom" {
...
func mockISSections(sl report.IncomeStatementSectionLabels) []ISStatementSection {
	rev := sl.Revenue
	if rev == "" {
		rev = "REVENUE"
	}
