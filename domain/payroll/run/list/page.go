...
// Row is the view-model for a single payroll run row (mapped from proto).
type Row struct {
	ID              string
	RunNumber       string
...
func fetchPayrollRuns(ctx context.Context, deps *Deps) []Row {
	...
	rows := make([]Row, 0, len(resp.GetPayrollRunList()))
...
func protoToRow(r *payrollrunpb.PayrollRun) Row {
...
func buildTableConfig(deps *Deps, status string, runs []Row, perms *types.UserPermissions) *types.TableConfig {
	l := deps.Labels
...
func payrollRunColumns(l run.Labels) []types.TableColumn {
	c := l.Columns
...
func buildTableRows(runs []Row, status string, l run.Labels, routes run.Routes, perms *types.UserPermissions) []types.TableRow {
	rows := []types.TableRow{}
	for _, r := range runs {
		...
		actions := []types.TableAction{
			{
				...
				Disabled: !canView, DisabledTooltip: l.Actions.NoPermission,
			},
		}
...
func statusTitle(l run.Labels, status string) string {
	switch status {
	case "draft":
		return l.Page.HeadingDraft
	case "calculated":
		return l.Page.HeadingCalculated
	case "approved":
...
func statusSubtitle(l run.Labels, status string) string {
	switch status {
	case "draft":
		return l.Page.SubtitleDraft
	case "calculated":
		return l.Page.SubtitleCalculated
	case "approved":
		return l.Page.SubtitleApproved
	case "posted":
		return l.Page.SubtitlePosted
	default:
		return l.Page.SubtitleDraft
	}
}

func statusLabel(l run.Labels, status string) string {
	switch status {
	case "draft":
		return l.Tabs.Draft
	case "calculated":
		return l.Tabs.Calculated
	case "approved":
