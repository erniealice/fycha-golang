...
// Row is the view-model for a single remittance row.
type Row struct {
	ID              string
...
func fetchRemittances(ctx context.Context, deps *Deps) []Row {
	...
	rows := make([]Row, 0, len(resp.GetPayrollRemittanceList()))
...
func protoToRow(r *payrollremittancepb.PayrollRemittance) Row {
	return Row{
		ID:              r.GetId(),
...
func buildTableConfig(deps *Deps, status string, remittances []Row, perms *types.UserPermissions) *types.TableConfig {
	l := deps.Labels
...
func remittanceColumns(l remittance.Labels) []types.TableColumn {
	c := l.Columns
	return []types.TableColumn{
		{Key: "remittance_type", Label: c.RemittanceType, WidthClass: "col-4xl"},
		{Key: "amount", Label: c.Amount, WidthClass: "col-3xl", Align: "right"},
		{Key: "due_date", Label: c.DueDate, WidthClass: "col-3xl"},
		{Key: "status", Label: c.Status, WidthClass: "col-xl"},
		{Key: "filed_at", Label: c.FiledAt, WidthClass: "col-3xl"},
		{Key: "reference_number", Label: c.ReferenceNumber, NoSort: true},
	}
}

func buildTableRows(remittances []Row, status string, l remittance.Labels, perms *types.UserPermissions) []types.TableRow {
	rows := []types.TableRow{}
...
func remittanceTypeLabel(l remittance.Labels, t string) string {
	switch t {
	case "sss":
		return l.Types.SSS
	case "philhealth":
		return l.Types.PhilHealth
	case "pagibig":
		return l.Types.PagIBIG
	case "bir_withholding":
		return l.Types.BIRWithholding
	default:
		return t
	}
}
...
func remittanceStatusLabel(l remittance.Labels, status string) string {
	switch status {
	case "pending":
		return l.Tabs.Pending
	case "filed":
		return l.Tabs.Filed
	case "paid":
		return l.Tabs.Paid
...
func remittanceStatusTitle(l remittance.Labels, status string) string {
	switch status {
	case "pending":
		return l.Page.HeadingPending
	case "filed":
		return l.Page.HeadingFiled
	case "paid":
...
func remittanceStatusSubtitle(l remittance.Labels, status string) string {
	switch status {
	case "pending":
		return l.Page.SubtitlePending
	case "filed":
		return l.Page.SubtitleFiled
	case "paid":
		return l.Page.SubtitlePaid
	default:
		return l.Page.SubtitlePending
	}
}
