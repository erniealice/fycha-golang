// Package payrolldashboard implements the read-only Payroll live dashboard view
...
type Deps struct {
	// Cross-entity route types — finalize agent will replace with sibling imports.
...
func buildRecentRunsHTML(runs []*payrollrunpb.PayrollRun, l PayrollDashboardLabels, _ PayrollRunRoutes) template.HTML {
	if len(runs) == 0 {
		return ""
	}
...
func buildRemittanceList(items []*payrollremittancepb.PayrollRemittance, l PayrollDashboardLabels) []types.ActivityItem {
	if len(items) == 0 {
		return nil
	}
