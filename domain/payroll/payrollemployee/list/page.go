...
// Row is the view-model for a single employee row.
type Row struct {
	ID           string
	Name         string
	Position     string
...
func employeeColumns(l payrollemployee.Labels) []types.TableColumn {
	c := l.Columns
...
func buildTableRows(employees []Row, l payrollemployee.Labels, perms *types.UserPermissions) []types.TableRow {
	rows := []types.TableRow{}
...
func payFrequencyLabel(l payrollemployee.Labels, freq string) string {
	switch freq {
	case "semi-monthly":
		return l.PayFrequency.SemiMonthly
	case "monthly":
		return l.PayFrequency.Monthly
	case "weekly":
		return l.PayFrequency.Weekly
	default:
		return freq
	}
}
