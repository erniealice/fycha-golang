...
func buildRecentRunsHTML(runs []*payrollrunpb.PayrollRun, l payroll.PayrollDashboardLabels, _ payroll.PayrollRunRoutes) template.HTML {
	if len(runs) == 0 {
		return ""
	}
...
func buildRemittanceList(items []*payrollremittancepb.PayrollRemittance, l payroll.PayrollDashboardLabels) []types.ActivityItem {
	if len(items) == 0 {
		return nil
	}
