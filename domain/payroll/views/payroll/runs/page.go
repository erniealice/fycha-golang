...
func payrollRunColumns(l payroll.PayrollLabels) []types.TableColumn {
	c := l.Run.Columns
...
func buildTableRows(runs []PayrollRunRow, status string, l payroll.PayrollLabels, routes payroll.PayrollRunRoutes, perms *types.UserPermissions) []types.TableRow {
	rows := []types.TableRow{}
	for _, r := range runs {
...
func statusTitle(l payroll.PayrollLabels, status string) string {
	switch status {
	case "draft":
...
func statusSubtitle(l payroll.PayrollLabels, status string) string {
	switch status {
	case "draft":
...
func statusLabel(l payroll.PayrollLabels, status string) string {
	switch status {
	case "draft":
