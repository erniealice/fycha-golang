...
type ViewDeps struct {
	Routes                lapsingschedule.Routes
	AssetRoutes           assetentity.Routes
...
type PageData struct {
	...
	AsOfDateMax     string
	RefreshURL      string
	Labels          depreciationrun.LapsingScheduleLabels
	CommonLabels    pyeza.CommonLabels
}
...
func lapsingScheduleColumns(l depreciationrun.LapsingScheduleLabels) []types.TableColumn {
	lc := l.Columns
...
func resolveStatusBadge(l depreciationrun.Labels, r CandidateRow) (label, variant string) {
	ls := l.LapsingSchedule
	switch r.Status {
	case "up_to_date":
