...
func depreciationRunColumns(l asset.DepreciationRunLabels) []types.TableColumn {
	lc := l.List.Columns
...
func buildTableRows(rows []drshared.DepreciationRunRow, l asset.DepreciationRunLabels, routes asset.DepreciationRunRoutes, perms *types.UserPermissions) []types.TableRow {
...
func statusBadge(l asset.DepreciationRunLabels, status string) (label, variant string) {
	switch status {
	case "pending":
...
func scopeKindLabel(l asset.DepreciationRunLabels, kind string) string {
	switch kind {
	case "asset":
...
func statusPageTitle(l asset.DepreciationRunLabels, status string) string {
	switch status {
	case "pending":
...
func statusEmptyTitle(l asset.DepreciationRunLabels, status string) string {
	switch status {
	case "pending":
...
func statusEmptyMessage(l asset.DepreciationRunLabels, status string) string {
	switch status {
	case "pending":
