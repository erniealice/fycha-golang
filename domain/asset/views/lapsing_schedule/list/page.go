...
type ViewDeps struct {
	Routes                asset.LapsingScheduleRoutes
...
type PageData struct {
	...
	AsOfDateMax     string
	RefreshURL      string
	Labels          asset.DepreciationRunLapsingScheduleLabels
	CommonLabels    pyeza.CommonLabels
}
...
func lapsingScheduleColumns(l asset.DepreciationRunLapsingScheduleLabels) []types.TableColumn {
	lc := l.Columns
...
func resolveStatusBadge(l asset.DepreciationRunLabels, r CandidateRow) (label, variant string) {
	ls := l.LapsingSchedule
	switch r.Status {
	case "up_to_date":
