...
func buildRecentPaymentsList(payments []*loanpaymentpb.LoanPayment, l treasury.LoanDashboardLabels) []types.ActivityItem {
	if len(payments) == 0 {
		return nil
	}
...
func buildTopLoansHTML(rows []LoanRow, l treasury.LoanDashboardLabels, routes treasury.LoanRoutes) template.HTML {
	if len(rows) == 0 {
		return ""
	}
