...
func buildTableConfig(deps *Deps, loans []LoanRow, status string, perms *types.UserPermissions) *types.TableConfig {
	...
	columns := []types.TableColumn{
		{Key: "loan_number", Label: l.Columns.LoanNumber, NoSort: true, WidthClass: "col-2xl"},
		{Key: "lender", Label: l.Columns.LenderName, NoSort: true},
		{Key: "type", Label: l.Columns.LoanType, NoSort: true, WidthClass: "col-2xl"},
		...
		{Key: "rate", Label: l.Columns.InterestRate, NoSort: true, WidthClass: "col-md", Align: "right"},
		{Key: "maturity", Label: l.Columns.MaturityDate, NoSort: true, WidthClass: "col-2xl"},
	}

	rows := []types.TableRow{}
	for _, loanRow := range loans {
		actions := []types.TableAction{
			{Type: "view", Label: l.Actions.View, Action: "view", Href: route.ResolveURL(deps.Routes.DetailURL, "id", loanRow.ID)},
		}

		typeVariant := loanTypeBadgeVariant(loanRow.LoanType)

		row := types.TableRow{
			ID:   loanRow.ID,
			Href: route.ResolveURL(deps.Routes.DetailURL, "id", loanRow.ID),
			Cells: []types.TableCell{
				{Type: "text", Value: loanRow.LoanNumber},
				{Type: "text", Value: loanRow.LenderName},
				{Type: "badge", Value: loanRow.LoanType, Variant: typeVariant},
				{Type: "money", Value: loanRow.PrincipalAmount},
				{Type: "money", Value: loanRow.RemainingBalance},
				{Type: "text", Value: loanRow.InterestRate},
				{Type: "text", Value: loanRow.MaturityDate},
			},
			Actions: actions,
		}
		rows = append(rows, row)
...
func headingForStatus(l loan.Labels, status string) (heading, caption string) {
	switch status {
	case "completed":
