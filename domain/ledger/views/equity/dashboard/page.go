...
func buildRecentTxnList(txns []*equitytransactionpb.EquityTransaction, l ledger.EquityDashboardLabels) []types.ActivityItem {
	if len(txns) == 0 {
		return nil
	}
...
func buildTopContributorsHTML(rows []EquityAccountRow, l ledger.EquityDashboardLabels, _ ledger.EquityRoutes) template.HTML {
	if len(rows) == 0 {
		return ""
	}
