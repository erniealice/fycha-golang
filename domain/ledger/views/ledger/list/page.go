...
func protoToRow(a *accountpb.Account, l ledger.AccountLabels) AccountRow {
	return AccountRow{
		ID:             a.GetId(),
		Code:           a.GetCode(),
		Name:           a.GetName(),
...
func accountColumns(l ledger.AccountLabels) []types.TableColumn {
...
func buildTableRows(accounts []AccountRow, element string, l ledger.AccountLabels, routes ledger.AccountRoutes, perms *types.UserPermissions) []types.TableRow {
	rows := []types.TableRow{}
...
func elementLabel(l ledger.AccountLabels, element string) string {
	switch element {
	case "asset":
		return l.Tabs.Asset
	case "liability":
...
func BuildAccountTree(accounts []AccountRow, l ledger.AccountFormLabels) []AccountRow {
	if len(accounts) == 0 {
		return accounts
	}
