...
type Deps struct {
	Routes       ledger.LedgerSettingsRoutes
	Labels       ledger.RecurringTemplateLabels
...
func templateColumns(l ledger.RecurringTemplateLabels) []types.TableColumn {
...
func buildTableRows(templates []RecurringTemplateRow, l ledger.RecurringTemplateLabels, perms *types.UserPermissions) []types.TableRow {
	rows := []types.TableRow{}
	for _, t := range templates {
...
func frequencyDisplayLabel(l ledger.RecurringTemplateLabels, frequency string) string {
	switch frequency {
	case "daily":
