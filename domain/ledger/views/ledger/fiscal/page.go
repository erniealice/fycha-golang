...
func periodColumns(l ledger.FiscalPeriodLabels) []types.TableColumn {
...
func buildTableRows(periods []FiscalPeriodRow, l ledger.FiscalPeriodLabels, routes ledger.FiscalPeriodRoutes, perms *types.UserPermissions) []types.TableRow {
	rows := []types.TableRow{}
	for _, p := range periods {
...
func statusDisplayLabel(l ledger.FiscalPeriodLabels, status string) string {
	switch status {
	case "open":
