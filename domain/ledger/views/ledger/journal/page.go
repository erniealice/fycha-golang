...
type Deps struct {
	Routes       ledger.JournalRoutes
	Labels       ledger.JournalLabels
...
func protoToRow(sl ledger.JournalSourceTypeLabels, e *jepb.JournalEntry) JournalRow {
	return JournalRow{
		ID:          e.GetId(),
...
func journalColumns(l ledger.JournalLabels) []types.TableColumn {
...
func buildTableRows(entries []JournalRow, status string, l ledger.JournalLabels, routes ledger.JournalRoutes, perms *types.UserPermissions) []types.TableRow {
	rows := []types.TableRow{}
...
func headingForStatus(l ledger.JournalLabels, status string) (heading, subtitle string) {
	switch status {
	case "draft":
...
func statusLabel(l ledger.JournalLabels, status string) string {
	switch status {
	case "draft":
		return l.Tabs.Draft
	case "posted":
...
func sourceTypeLabel(l ledger.JournalSourceTypeLabels, t jepb.JournalSourceType) string {
	switch t {
