...
func protoToRow(sl journalpkg.SourceTypeLabels, e *jepb.JournalEntry) JournalRow {
	return JournalRow{
		ID:          e.GetId(),
...
func journalColumns(l journalpkg.Labels) []types.TableColumn {
...
func buildTableRows(entries []JournalRow, status string, l journalpkg.Labels, routes journalpkg.Routes, perms *types.UserPermissions) []types.TableRow {
	rows := []types.TableRow{}
...
func headingForStatus(l journalpkg.Labels, status string) (heading, subtitle string) {
	switch status {
	case "draft":
...
func statusLabel(l journalpkg.Labels, status string) string {
	switch status {
	case "draft":
		return l.Tabs.Draft
	case "posted":
