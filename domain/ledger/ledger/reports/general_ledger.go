...
type GeneralLedgerDeps struct {
	Routes       accountpkg.StatementRoutes
...
func buildGLSummary(s *GLAccountSection, labels accountpkg.Labels) []report.SummaryMetric {
...
func buildGLTable(s *GLAccountSection, tableLabels types.TableLabels, labels accountpkg.Labels) *types.TableConfig {
...
func mockGLSection(accountID, startDate, endDate, journalDetailURLTmpl string) *GLAccountSection {
	if journalDetailURLTmpl == "" {
		journalDetailURLTmpl = journalpkg.DetailURL
	}
