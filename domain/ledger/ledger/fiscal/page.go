...
type Deps struct {
	Routes       fiscalperiodpkg.Routes
	Labels       fiscalperiodpkg.Labels
...
func periodColumns(l fiscalperiodpkg.Labels) []types.TableColumn {
	return []types.TableColumn{
		{Key: "period", Label: l.Columns.Period, NoSort: true},
		{Key: "year", Label: l.Columns.Year, NoSort: true, WidthClass: "col-lg"},
		{Key: "start_date", Label: l.Columns.StartDate, NoSort: true, WidthClass: "col-2xl"},
		{Key: "end_date", Label: l.Columns.EndDate, NoSort: true, WidthClass: "col-2xl"},
		{Key: "status", Label: l.Columns.Status, NoSort: true, WidthClass: "col-xl"},
	}
}

func buildTableRows(periods []FiscalPeriodRow, l fiscalperiodpkg.Labels, routes fiscalperiodpkg.Routes, perms *types.UserPermissions) []types.TableRow {
	rows := []types.TableRow{}
	for _, p := range periods {
...
func statusDisplayLabel(l fiscalperiodpkg.Labels, status string) string {
	switch status {
	case "open":
