...
type Deps struct {
	Routes       recurringtemplatepkg.SettingsRoutes
	Labels       recurringtemplatepkg.Labels
...
func templateColumns(l recurringtemplatepkg.Labels) []types.TableColumn {
...
func buildTableRows(templates []RecurringTemplateRow, l recurringtemplatepkg.Labels, perms *types.UserPermissions) []types.TableRow {
	rows := []types.TableRow{}
	for _, t := range templates {
...
func frequencyDisplayLabel(l recurringtemplatepkg.Labels, frequency string) string {
	switch frequency {
	case "daily":
