...
func buildRecentJournalActivity(entries []*journalentrypb.JournalEntry, l accountpkg.DashboardLabels) []types.ActivityItem {
	if len(entries) == 0 {
		return nil
	}
...
func buildJournalsHTML(entries []*journalentrypb.JournalEntry, l accountpkg.DashboardLabels, routes journalpkg.Routes) template.HTML {
	if len(entries) == 0 {
		return ""
	}
