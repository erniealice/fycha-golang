...
func buildRecentTxnList(txns []*equitytransactionpb.EquityTransaction, l equity.DashboardLabels) []types.ActivityItem {
	if len(txns) == 0 {
		return nil
	}
...
func buildTopContributorsHTML(rows []EquityAccountRow, l equity.DashboardLabels, _ equity.Routes) template.HTML {
	if len(rows) == 0 {
		return ""
	}
