// Package fundinglabels holds the funding drawer-form label structs.
//
// These types live in a leaf package (rather than the domain/funding facade)
// so that both the domain/funding facade AND the funding leaf views can import
// them without creating an import cycle:
//
//	domain/funding (facade) -> domain/funding/funding (module) -> leaf views
//
// All three import this leaf; none imports the facade for these types.
//
// Lyngua root key: "funding"; JSON lives in
// packages/lyngua/translations/en/{general,professional}/funding.json.
package fundinglabels
